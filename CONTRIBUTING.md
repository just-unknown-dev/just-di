# Contributing to Just DI

Thank you for your interest in contributing to **Just DI**! We welcome all contributions — bug fixes, new features, documentation improvements, and more.
...
2. **Clone** your fork locally:
   ```bash
   git clone https://github.com/just-unknown-dev/just-di.git
   cd just_di
   ```
...
- Check that the feature aligns with the package's scope (lightweight, framework-agnostic dependency injection for Dart).
...
`just_di` is a pure-Dart package with no Flutter dependency. All commands run from `packages/just_di`.
...
- Keep the package framework-agnostic — avoid Flutter-specific APIs in `lib/src/`.
...
**Examples:**
```
feat(di): add async factory registration support
fix(di): resolve circular dependency detection for lazy singletons
...
By contributing to Just DI you agree that your contributions will be licensed under the [BSD-3-Clause License](LICENSE).
